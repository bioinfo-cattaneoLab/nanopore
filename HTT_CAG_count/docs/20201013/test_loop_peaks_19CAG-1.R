...
id_siCRE_noNEO=reads_CRE_noNEO %>% filter ( alignment_coverage != "-1" & alignment_genome_start < as.numeric(35) ) %>% select(read_id)
id_noCRE_noNEO=reads_CRE_noNEO %>% filter ( alignment_coverage != "-1" & alignment_genome_start > as.numeric(35) ) %>% select(read_id)

id_siCRE_siNEO=reads_CRE_siNEO %>% filter ( alignment_coverage != "-1" & alignment_genome_start < as.numeric(35) ) %>% select(read_id)
id_noCRE_siNEO=reads_CRE_siNEO %>% filter ( alignment_coverage != "-1" & alignment_genome_start > as.numeric(35) ) %>% select(read_id)
	...
	if(dim(count)[1] != 0) {
		...
		if(dim(peaks)[1] > 10){
			...
			if(dim(turningPosition)[1] != 0){
				#plots
				filenameTPpng=paste("~/projects/cattaneo/docs/20201013/alignment/plots/peaksTP_",c,".png",sep="")
				png(filenameTPpng, width = 17, height = 17, units="cm", res= 300)
				plot(peaks.tp)
				dev.off()
				filenamepng=paste("~/projects/cattaneo/docs/20201013/alignment/plots/peaks_",c,".png",sep="")
				...
				for ( num in as.numeric(row.names(turningPosition))) {
					#check if turningPosition contains almost 1 peak and two pits (?)
					if(nrow(df)<3) {
						n_count=peaks$count[turningPosition$tppos[num]]
						n_reads=peaks %>% select(n) %>% sum()
						test=data.frame(n_count, n_reads,c)
						df_to_build=rbind(df_to_build,test)
					}
					# ricavare la conta di CAG dal tppos
					if(nrow(df)>= 3){
					n_count=peaks$count[turningPosition$tppos[num]]
					# ricavare il numero di reads usando df$tppos[which(df$tppos == turningPosition$tppos[num])-+1] per ricavare la posizione subito prima e subito dopo il picco
					n_reads=peaks %>% filter(count > peaks$count[df$tppos[which(df$tppos == turningPosition$tppos[num])-1]], count < peaks$count[df$tppos[which(df$tppos == turningPosition$tppos[num])+1]]) %>% select(n) %>% sum()
					test=data.frame(n_count, n_reads,c)
					df_to_build=rbind(df_to_build,test)
					}
				}
...
colnames(df_to_build) <- c('CAG_count', 'n_reads','alignment CRE/NEO')
...
if(dim(df_to_build)[1] != 0) write.table(df_to_build,"~/test_CAG.tsv", quote=F, row.name=F, sep="\t")
