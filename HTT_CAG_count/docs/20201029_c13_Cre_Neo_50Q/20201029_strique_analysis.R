...
df_150_600bp=read.table("~/projects/cattaneo/striqueOutput/20201029_c13_Cre_Neo_50Q/20201029_allSAM_qscore7.50Q-p52.150_600bp_CAG.tsv",header=T, sep="\t")

df_150_600bp$lengthPS = "150bp"
df_150_600bp$lengthRef_interval = "600bp"
df_150_600bp$QSCORE = 7
df_150_600bp$replicate = "1"
df_150_600bp$experiment = "50Q-p52"

temp_name=read.table("~/projects/cattaneo/striqueOutput/20201029_c13_Cre_Neo_50Q/20201029_allSAM_qscore7.50Q-p67.150_600bp_CAG.tsv",header=T, sep="\t")
...
filePath=paste("~/projects/cattaneo/striqueOutput/20201029_c13_Cre_Neo_50Q/20201029_allSAM_qscore", QSCORE,".50Q-p", EXP,".150_600bp_CAG.tsv" , sep="")
...
final_df$experiment = factor(final_df$experiment, levels = c("50Q-p52", "50Q-p67"))

final_df %>% filter(count > 0 ) %>% ggplot(aes(x=count)) + geom_density(aes(color=experiment,linetype = QSCORE)) + ggtitle( "CAG count with different Qscore, 150bp Prefix/Suffix length" ) + facet_grid(  experiment ~., scales="free" )
...
# }
#
# final_df %>% filter(count > 0 & score_prefix > 6 & score_suffix > 6 ) %>%  group_by(replicate, QSCORE, lengthPS, lengthRef_interval) %>% summarize(mean=mean(count), median=median(count), sd=sd(count),mode=getmode(count))
#
#
#
# #CAG count Distribution
...
#
# final_df %>% filter (count > 0 & score_prefix > 6 & score_suffix > 6) %>% group_by(replicate, QSCORE, lengthPS, lengthRef_interval, reads_used) %>% summarize(mean=mean(count), median=median(count), sd=sd(count)) %>% ggplot( aes( replicate , mean )) + geom_point(aes(shape=lengthPS, color=as.factor(QSCORE), size = reads_used)) + ggtitle( "Mean of CAG count for lengthRef_interval (count >0)" )
#
# ggsave("~/projects/cattaneo/docs/20201013/plots/meanCAG_4_replicate.png")
#
# #Median of CAG count for lengthRef_interval
#
# final_df %>% filter (count > 0 & score_prefix > 6 & score_suffix > 6) %>% group_by(replicate, QSCORE, lengthPS, lengthRef_interval, reads_used) %>% summarize(mean=mean(count), median=median(count), sd=sd(count)) %>% ggplot( aes( replicate , median )) + geom_point(aes(shape=lengthPS, color=as.factor(QSCORE), size = reads_used)) + ggtitle( "Median of CAG count for lengthRef_interval (count >0)" )
#
# ggsave("~/projects/cattaneo/docs/20201013/plots/medianCAG_4_replicate.png")
#
# #Mode of CAG count
#
# final_df %>% filter (count > 0 & score_prefix > 6 & score_suffix > 6) %>%  group_by(replicate, QSCORE, lengthPS, lengthRef_interval, reads_used) %>% summarize(mean=mean(count), median=median(count), sd=sd(count), mode=getmode(count)) %>% ggplot( aes( replicate , mode )) + geom_point(aes(shape=lengthPS, color=as.factor(QSCORE), size = reads_used)) + ggtitle("Mode of CAG count for lengthRef_interval (count >0)")
#
# ggsave("~/projects/cattaneo/docs/20201013/plots/modeCAG_4_replicate.png")
