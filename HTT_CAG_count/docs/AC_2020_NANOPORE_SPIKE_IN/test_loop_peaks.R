...
for ( c in c("noCRE_noNEO","siCRE_noNEO","noCRE_siNEO","siCRE_siNEO")) {
	...
	#plots
	filenameTPpng=paste("~/projects/cattaneo/docs/AC_2020_NANOPORE_SPIKE_IN/alignment/plots/peaksTP_",c,".png",sep="")
	png(filenameTPpng, width = 17, height = 17, units="cm", res= 300)
	plot(peaks.tp)
	dev.off()
	filenamepng=paste("~/projects/cattaneo/docs/AC_2020_NANOPORE_SPIKE_IN/alignment/plots/peaks_",c,".png",sep="")
...
#write table with count, n_reads e type of alignment
write.table(df_to_build,"~/test_CAG.tsv", quote=F, row.name=F, sep="\t")
