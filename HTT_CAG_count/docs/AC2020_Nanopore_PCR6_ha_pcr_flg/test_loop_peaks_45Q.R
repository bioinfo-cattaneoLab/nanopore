...
#README : questa versione conta le reads che compongono il picco tenendo conto delle pits che lo circondano (non mi piace come e' applicato)
		...
		if(dim(peaks)[1] > 10){
			...
			if(dim(turningPosition)[1] != 0){
				...
				for ( num in as.numeric(row.names(turningPosition))) {
					...
					#if the last element in df is a good peak consider for n_reads only the pit before and not after(don't exist)
					if(nrow(df)>= 3 && ((df$tppos[which(df$tppos == turningPosition$tppos[num])+1]) %in% df$tppos) == FALSE){
						n_count=peaks$count[turningPosition$tppos[num]]
						...
						n_reads=peaks %>% filter(count > peaks$count[df$tppos[which(df$tppos == turningPosition$tppos[num])-1]], count == peaks$count[df$tppos[which(df$tppos == turningPosition$tppos[num])]]) %>% select(n) %>% sum()
						info=df %>% filter (tppos==turningPosition$tppos[num]) %>% select(info)
						test=data.frame(n_count, n_reads,c,info)
						df_to_build=rbind(df_to_build,test)
					}
					#if the first element in df is a good peak consider for n_reads only the pit after and not before(don't exist)
					if(nrow(df)>= 3 && ((df$tppos[which(df$tppos == turningPosition$tppos[num])+1]) %in% df$tppos) == TRUE && length((df$tppos[which(df$tppos == turningPosition$tppos[num])-1])) == 0){
						n_count=peaks$count[turningPosition$tppos[num]]
						# ricavare il numero di reads usando df$tppos[which(df$tppos == turningPosition$tppos[num])-+1] per ricavare la posizione subito prima e subito dopo il picco
						n_reads=peaks %>% filter(count < peaks$count[df$tppos[which(df$tppos == turningPosition$tppos[num])+1]], count == peaks$count[df$tppos[which(df$tppos == turningPosition$tppos[num])]]) %>% select(n) %>% sum()
						info=df %>% filter (tppos==turningPosition$tppos[num]) %>% select(info)
