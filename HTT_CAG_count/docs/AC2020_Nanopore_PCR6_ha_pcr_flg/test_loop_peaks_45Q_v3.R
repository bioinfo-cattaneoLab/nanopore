...
#README : questa versione conta le reads che compongono il picco considerando anche il numero di reads che stanno a distanza +-2 dal picco detectato da PASTECS. Si potrebbero contare due volte le stesse reads perche' due picchi potrebbero stare alla distanza minore di 2
		...
		if(dim(peaks)[1] > 10){
			...
			if(dim(turningPosition)[1] != 0){
				...
				for ( num in as.numeric(row.names(turningPosition))) {
					#obtain n_reads of peaks (+-1) , n_count associated with this peaks and info value associated with
					n_count=peaks$count[turningPosition$tppos[num]]
					n_reads = sum(peaks$n[turningPosition$tppos[num]-(1:2)] , peaks$n[turningPosition$tppos[num]], peaks$n[turningPosition$tppos[num]+(1:2)])
