...
final_df %>% filter (count > 0) %>% group_by(QSCORE, lengthPS, lengthRef_interval, tested_reads) %>% summarize(mean=mean(count), median=median(count), sd=sd(count)) %>% ggplot( aes( lengthRef_interval , median )) + geom_point(aes(shape=lengthPS, color=as.factor(QSCORE), size = tested_reads)) + ggtitle( "Median of CAG count for lengthRef_interval (count >0)" )
...
final_df %>% filter (count > 0) %>% group_by(QSCORE, lengthPS, lengthRef_interval, tested_reads) %>% summarize(mean=mean(count), median=median(count), sd=sd(count)) %>% ggplot( aes( lengthRef_interval , mean )) + geom_point(aes(shape=lengthPS, color=as.factor(QSCORE), size = tested_reads)) + ggtitle( "Mean of CAG count for lengthRef_interval (count >0)" )
...
final_df %>% filter (count > 0) %>%  group_by(QSCORE, lengthPS, lengthRef_interval, tested_reads) %>% summarize(mean=mean(count), median=median(count), sd=sd(count), mode=getmode(count)) %>% ggplot( aes( lengthRef_interval , mode )) + geom_point(aes(shape=lengthPS, color=as.factor(QSCORE), size = tested_reads)) + ggtitle("Mode of CAG count for lengthRef_interval (count >0)")
